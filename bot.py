...
def get_token(tta_data):
    user_id = tta_data["id"]
    result = SQL_request("SELECT name FROM sqlite_master WHERE type='table' AND name='github_tokens';")
    if not result:
        create_tokens()
    token_data = SQL_request("SELECT * FROM github_tokens WHERE user_id=?", (user_id,), 'one')
    if token_data:
...
def get_gists(tta_data):
    ...
    try:
        gists = fetch_github_data(f"https://api.github.com/users/{owner}/gists", tta_data)
        return {f'gist|{g["id"]}': g["description"] for g in gists}
    except:
        return {"error": "Гисты не найдены"}

def get_my_gists(tta_data):
...
def save_token(tta_data):
    ...
    if token:
        token_data = SQL_request("SELECT * FROM github_tokens WHERE user_id=?", (user_id,), 'one')
        if token_data:
            SQL_request("UPDATE github_tokens SET token = ? WHERE user_id = ?", (token, user_id))
        else:
            SQL_request('INSERT INTO github_tokens (user_id, token) VALUES (?, ?)', (user_id, token))
            return
    else:
...
def get_user_token(tta_data):
    user_id = tta_data["id"]
    token = SQL_request("SELECT token FROM github_tokens WHERE user_id=?", (user_id,), 'one')
    if not token:
